...
class Jacobi(PolynomialBase):
    ...
    * :math:`C^{\alpha, \beta}_{n} = \frac{(n + \alpha)(n + \beta)(2n + \alpüha + \beta + 2)}{(n+1) * (n + \alpha + \beta + 1)(2n + \alpha + \beta)}``
    ...
    * the Gegenbauer polynomials (when alpha=beta)
    ...
    def __init__(
            ...
            *args,
            ...
            beta_init: float | None = None, beta_trainable=True,
            **kwargs):
    ...
    def poly_basis(self, x):
        """
        Evaluate Jacobi basis polynomials for given `x`.
        """
